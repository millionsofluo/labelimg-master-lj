...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        self.autoSaving.setChecked(True)  # 改成一直自动保存
    ...
    def showFileInfo(self, ind, total_num):
        '''
        在file list控件上显示当前第几张图片和图片总数
        '''
        # print self.filedock.windowTitle();
        # self.filedock.setWindowTitle("list");
        self.filedock.setWindowTitle(
            u'File List: {0}/{1}'.format(ind, total_num))
    ...
    def fileitemDoubleClicked(self, item=None):
        ...
        # 去掉序号，要不然没法跳转
        clicked_item = ustr(item.text())
        slice_i = 0
        for i, chris in enumerate(clicked_item):
            if chris == ':':
                slice_i = i+1
                break
        clicked_img_path = clicked_item[slice_i:]

        currIndex = self.mImgList.index(clicked_img_path)
        if currIndex < len(self.mImgList):
            filename = self.mImgList[currIndex]
            self.showFileInfo(currIndex+1, len(self.mImgList))
            if filename:
    ...
    def importDirImages(self, dirpath):
        ...
        for idx, imgPath in enumerate(self.mImgList):
            # file list 那里加上图片序号
            item = QListWidgetItem(str(idx+1)+':'+imgPath)
            self.fileListWidget.addItem(item)
        self.showFileInfo(1, len(self.mImgList))
    ...
    def openPrevImg(self, _value=False):
        ...
        if currIndex - 1 >= 0:
            ...
            self.showFileInfo(currIndex, len(self.mImgList))
            if filename:
                self.loadFile(filename)
    ...
    def openNextImg(self, _value=False):
        ...
        if self.filePath is None:
            ...
            if currIndex + 1 < len(self.mImgList):
                filename = self.mImgList[currIndex + 1]
                self.showFileInfo(currIndex+2, len(self.mImgList))
            # 到头以后返回第一张
            if currIndex + 1 == len(self.mImgList):
                filename = self.mImgList[0]
                self.showFileInfo(1, len(self.mImgList))
    ...
    def loadPredefinedClasses(self, predefClassesFile):
        ...
        读取标签文件
        '''
        # self.labelHist = ["Ground_wire"]
        # self.labelHist = ["shu","heng","dao"]
        # self.labelHist = ["xiushi","xiaodingqueshi"]
        # self.labelHist = ["lineYes","lineNo"]
        ...
        # self.labelHist = ["避雷器1", "避雷器2", "油位表", "temp2", "temp3", "temp4"]
        self.labelHist = ["避雷器1_数字框", "避雷器1_表盘",
                          "避雷器1_指针", "避雷器2_数字框", "避雷器2_指针", "temp4"]
